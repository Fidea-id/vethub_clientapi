...
namespace Infrastructure.Repositories
{
    public class GenericRepository<T, TFilter> : IGenericRepository<T, TFilter> where T : class where TFilter : BaseEntityFilter
    {
        ...
        public async Task<IEnumerable<T>> WhereQueryDict(string dbName, Dictionary<string, object> filters)
        {
            using (var _db = _dbFactory.GetDbConnection(dbName))
            {
                var conditions = string.Join(" AND ", filters.Keys.Select(k => $"{k} = @{k}"));
                var sql = $"SELECT * FROM {_tableName} WHERE {conditions}";

                return await _db.QueryAsync<T>(sql, filters);
            }
        }
